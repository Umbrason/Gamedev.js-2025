...
public static class NetworkUtils
{
    ...
    public static bool DistributedRandomInts(this INetworkChannel networkChannel, string header, ref Dictionary<PlayerID, int> results)
    {
        if (results != null) return results.Count >= playerCount;
        ...
        void ResultCallback(NetworkMessage message)
        {
            dict[message.sender] = (int)message.content;
            if (dict.Count >= playerCount) networkChannel.StopListening(header);
        }
        var value = RNG.Next();
        results[networkChannel.PlayerID] = value;
    ...
    /// </summary>
    static public int CombineRandomInts(this Dictionary<PlayerID, int> ints)
    {
        int bits = 0;

        foreach ((var _, int i) in ints)
        {
            bits ^= i;
        }
